...
/*   Updated: 2020/02/24 15:56:47 by lloncham         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_collision		ray_hit_triangle(t_ray *ray, t_collide_triangle *collidable)
{
	...
	return ((t_collision) {
		.collide = TRUE,
		.dist = t,
		.uv = ft_vec2_add(ft_vec2_add(ft_vec2_mul_s(collidable->uv[0], 1.0 - u - v), ft_vec2_mul_s(collidable->uv[1], u)),  ft_vec2_mul_s(collidable->uv[2], v)),
		.ray = *ray,
		.who = (t_collidable){
