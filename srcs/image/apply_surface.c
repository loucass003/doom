...
/*   Updated: 2019/10/20 21:26:38 by llelievr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	SDL_Rect dst)
{
	...
	while (++i < s_p.y)
	{
		...
		while (++j < s_p.x)
		{
			if (!put_pixel(img, j, i, ft_color_i(ft_alpha_blend(ft_i_color(
				get_surface_pixel(s, (int)((j - d2.x) * s_v.x) + src.x,
					(int)((i - d2.y) * s_v.y) + src.y)),
				ft_i_color(img->pixels[j + (i * img->width)])))))
				break;
		}
	}
}
