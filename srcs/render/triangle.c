...
/*   Updated: 2020/01/10 17:29:35 by llelievr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void scanline2(t_render_context *ctx, t_render_data *data, t_pixel p, float t, t_vertex start, t_vertex end)
{
	t_vertex	vert;
	float		*buff;
...
//	pthread_mutex_lock(&ctx->doom->mutex);
	...
	if (vert.pos.w <= *buff)
	{
		...
		if (ctx->mode == RM_DIFFUSE || (ctx->mode == RM_DEPTHBUFFER && data->mtl->transparent))
		{
...
void line(t_render_context *ctx, t_render_data *data, t_vertex start, t_vertex end, int i)
{
	...
	{
		scanline2(ctx, data, (t_pixel){ j, i, 0 }, t, start, end);
		t += tstep;
	}
}
