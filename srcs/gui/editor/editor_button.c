...
/*   Updated: 2020/01/24 15:20:44 by lloncham         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void					g_editor_button(t_gui *self, t_doom *doom)
{
	...
	append_components_array(&self->components, create_button((SDL_Rect)
		{8, 9, 50, 50}, "icons/icons8-jouer-80.png", NULL));
	append_components_array(&self->components, create_button((SDL_Rect)
		{60, 9, 50, 50}, "icons/poly.png", NULL));
	append_components_array(&self->components, create_button((SDL_Rect)
		{112, 9, 50, 50}, "icons/point.png", NULL));
	append_components_array(&self->components, create_button((SDL_Rect)
		{164, 9, 50, 50}, "icons/curseur3.png", NULL));
	append_components_array(&self->components, create_button((SDL_Rect)
		{216, 9, 50, 50}, "icons/objects.png", NULL));
	append_components_array(&self->components, create_button((SDL_Rect)
		{268, 9, 50, 50}, "icons/pin.png", NULL));
	i = 0;
	while (i < 6)
		self->components->values[i++]->perform_action = tools_action_performed;
	append_components_array(&self->components, create_button((SDL_Rect)
		{S_WIDTH - 57, 9, 50, 50}, "icons/settings.png", NULL));
	self->components->values[self->components->len - 1]
		->perform_action = action_performed;
	((t_button *)self->components->values[self->components->len - 1])
		->selected = doom->editor.settings.open;
}
