...
static void		action_performed(t_component *cmp, t_doom *doom)
{
	...
	else if (cmp == doom->guis[doom->current_gui].components[8]) //etages
	{
		doom->editor.floor++;
		doom->editor.click = 0;
	}
	...
	else if (cmp == doom->guis[doom->current_gui].components[10]) //lignes
	{
		...
		erase_all(doom);
	else if (cmp == doom->guis[doom->current_gui].components[11])
		doom->editor.secteur = (doom->editor.secteur == 0) ? 1 : 0;
	else if (cmp == doom->guis[doom->current_gui].components[12])
		doom->editor.porte = (doom->editor.porte == 0) ? 1 : 0;
}

void			g_editor_button(t_gui *self, t_doom *doom)
{
	if (!alloc_components(self, 13))
		return ;
	...
	{326, 6, 40, 40}, "icons/icons8-chevron-haut-80.png");
	self->components[9] = create_button((SDL_Rect)
	{406, 6, 40, 40}, "icons/icons8-chevron-bas-80.png");
	self->components[10] = create_button((SDL_Rect)
	{446, 6, 40, 40}, "icons/icons8-stylo-64.png");
	self->components[11] = create_button((SDL_Rect)
	{490, 6, 40, 40}, "icons/icons8-modifier-la-ligne-80.png");
	self->components[12] = create_button((SDL_Rect)
