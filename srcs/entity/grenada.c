...
/*   Updated: 2020/02/16 17:38:15 by lloncham         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool		create_explosion_renderable(t_doom *doom, t_renderable *r)
{
	...
	if (!(sprite = create_sprite((t_vec2){ 8, 6 },
		doom->res_manager.ressources->values[24])))
		return (FALSE);
...
t_bool		update_renderable_explosion(t_render_context *ctx, t_renderable *r)
{
	...
	if (e->t0 >= 1)
	{
		...
		{
			splice_renderables_array(ctx->doom->renderables,
				renderables_indexof(ctx->doom->renderables, r), 1);
			return (FALSE);
		}
