...
/*   Updated: 2019/11/25 17:16:40 by llelievr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool		read_model(t_ressource_manager *r, t_renderable **m)
{
    ...
    if (!(model->faces = create_faces_array(wr_model.faces_count)) 
        ...
        || !(model->vertex = create_2dvertices_array(wr_model.vertex_count + 1))
    ...
    if (!io_memcpy(&r->reader, model->faces->values, wr_model.faces_count * sizeof(t_face))
        ...
        || !io_memcpy(&r->reader, model->vertex->vertices, wr_model.vertex_count * sizeof(t_vec2)))    
        return (FALSE);
    i = -1;
    while (++i < wr_model.materials_count)
    {
